...
- [x] T042 [P] [US6] Unit test `tests/unit/initiative_probe.bats` — `initiative::probe_available` returns `present`/`absent` from the issue-type metadata probe; `ensure_initiative` creates the Initiative (narrative only from the explicit `spec_input` source, never inferred) when present (Q5, FR-014)
- [x] T043 [P] [US6] Unit test `tests/unit/initiative_degrade.bats` — `initiative::degrade_onto_epic` folds the narrative onto the Epic behind a stable marker and carries repo grouping via the reused `repo_prefix` label when absent; never hard-fails; re-runs in the degraded state are zero-churn and a later re-home onto a real Initiative is churn-free (Q5, FR-013, SC-007)
- [x] T044 [US6] Implement `initiative::probe_available` + `ensure_initiative` in `src/jira_sink.sh` — gated on `mapping.initiative.enabled`; require `issue_types.initiative` only when present; populate narrative only from the explicit `spec_input` source (FR-013, FR-014)
- [x] T045 [US6] Implement `initiative::degrade_onto_epic` in `src/jira_sink.sh` — stable-marker fold onto the Epic + reused `repo_prefix` grouping label; idempotent degrade↔re-home; `spec→Story` stays the sole backward-drift anchor (FR-010, Q5)
- [x] T046 [US6] Wire the Initiative super-level into `src/reconcile.sh` orchestration — when enabled, probe then create-or-degrade above the repo Epic; `spec_input` gracefully absent in `--workstate` mode (Q8 edge case)
- [x] T047 [US6] Integration test `tests/integration/us6_initiative.bats` — super-level on + Initiative present ⇒ an Initiative is created above the Epic; on + absent ⇒ narrative folds onto the Epic behind a marker, repo grouping becomes a label, run succeeds; super-level off ⇒ behavior matches US1 (spec scenarios 1–3, SC-007)
...
- [x] T048 [P] shellcheck `--severity=style` clean across all touched `src/*.sh`; fix findings
- [x] T049 [P] yamllint `-d relaxed` clean on the updated `config-template.yml` + any workflow change
- [x] T050 [P] markdownlint-clean across `specs/002-configurable-mapping/**/*.md` (`npx markdownlint-cli2`)
- [x] T051 [P] Extend `tests/unit/no-real-identifiers.bats` coverage over the new fixtures (`issuetype_meta/`, `workstate/direct/`, the template `mapping:` block); confirm placeholders only (FR-019, Privacy IX)
- [x] T052 [P] Update `CHANGELOG.md` (Unreleased: configurable artifact mapping — alias default, per-level mapping + validation, 2-level checklist, status rollup, workstate-direct, initiative super-level)
- [x] T053 [P] Validate `specs/002-configurable-mapping/quickstart.md` against the shipped behavior (modes, validation, workstate-direct) and correct any drift
- [x] T054 Run the exact CI locally via `scripts/check.sh` (shellcheck + yamllint + markdownlint + bats unit) and fix to green before pushing
