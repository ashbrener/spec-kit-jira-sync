#!/usr/bin/env bats
# =============================================================================
# tests/unit/drift_detection.bats — foundational drift units for the engine.
#
# Ported from spec-kit-linear @ 7dbe6bd (Linear→neutral/Jira terms). These
# prove the COPIED drift hardening survived extraction. Covers the PURE drift
# machinery (no sink writes, no Jira network):
#   * git_helpers::iso_to_epoch        — cross-platform ISO→epoch
#   * git_helpers::spec_dir_last_commit — recency disk key
#   * git_helpers::worktrees_touching_spec — multi-worktree ranking
#   * reconcile::_phase_ordinal        — lifecycle ladder
#   * reconcile::compute_drift         — pure comparator
#
# Each test runs in a hermetic temp git repo. The compute_drift fixtures live
# in tests/fixtures/drift_responses/. These are PURE-unit tests — no Jira
# network, no writes.
# =============================================================================
...
FIXTURES="${REPO_ROOT}/tests/fixtures/drift_responses"
...
@test "spec_dir_last_commit: returns the committing ISO date for a touched dir" {
  ...
  [ "$status" -eq 0 ]
  [ -n "$output" ]
  # The returned string MUST parse to a sane epoch via the converter.
...
# -----------------------------------------------------------------------------
# reconcile::compute_drift — no-drift / forward cases
# -----------------------------------------------------------------------------
...
@test "compute_drift: disk merged, tracker behind at implementing fires nothing (US1)" {
...
@test "compute_drift: absent tracker issue fires nothing (first reconcile)" {
...
@test "compute_drift: uninferrable disk phase disables phase signal (recency-only)" {
  # Disk commit equal to tracker updatedAt; disk phase unknown → phase skipped,
...
# -----------------------------------------------------------------------------
# reconcile::compute_drift — drift-fired cases
# -----------------------------------------------------------------------------

@test "compute_drift: phase-only fire (recency unavailable) names phase_ordering" {
  # No committed spec dir → recency unavailable; tracker implementing > disk planning.
  local json; json="$(cat "$FIXTURES/spec_issue_tracker_ahead_phase.json")"
...
@test "compute_drift: equal phase + tracker updatedAt far ahead fires NOTHING (idempotency)" {
  # Recency is a CORROBORATING signal only — it never fires on its own.
  # disk commit 09:31:00, tracker updatedAt 09:35:00 (+240s) but BOTH planning
  # (equal phase). This is the shape of a no-op re-run after the bridge's own
  # write bumped updatedAt; the contract suppresses it: no phase drift to
  # corroborate ⇒ nothing fires.
  _commit_spec_at "specs/311-recency" "2026-05-26T09:31:00+00:00"
  local json; json="$(cat "$FIXTURES/spec_issue_tracker_ahead_recency.json")"
...
@test "compute_drift: both signals fire → signals=phase_ordering,recency" {
  # disk planning committed long ago; tracker implementing + much newer.
...
@test "compute_drift: skew tolerance is overridable via the environment" {
  # Recency only corroborates a phase drift, so keep a phase drift live
  # (disk planning < tracker implementing) and vary only whether recency
  # piggybacks. disk commit 09:28:00, tracker updatedAt 09:31:00 (+180s). A
...
# -----------------------------------------------------------------------------
# git_helpers::worktrees_touching_spec ranking
# -----------------------------------------------------------------------------
...
@test "worktrees_touching_spec: epoch tie → invoking worktree canonical, both in the touching set" {
  ...
  # on a tie; both still appear in the set.
...
@test "worktrees_touching_spec: epoch tie → LINKED invoking worktree wins over porcelain-first main" {
  # Invoke from the LINKED worktree, which is NOT porcelain-first, so the only
  # way it can appear first is the invoking-worktree reorder logic firing.
  ...
  # porcelain output (macOS /var → /private/var resolution).
  ...
  # worktree — otherwise this test would prove nothing.
  ...
  # FIRST — line 0 is the 008-tie feature path, NOT main.
