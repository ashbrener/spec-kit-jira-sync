#!/usr/bin/env bats
# =============================================================================
# tests/unit/drift_disposition.bats — arg-parse, summary, WARNING + fork.
#
# Ported from spec-kit-linear @ 7dbe6bd (Linear→neutral/Jira terms). These
# prove the COPIED disposition hardening survived extraction. Covers the pure,
# network-free pieces of the engine's drift write-path spine:
#   * --on-drift=abort|proceed parsing into ARG_ON_DRIFT
#   * --on-drift bad value → usage error at parse time
#   * --retroactive deprecation INFO row, exactly once
#   * summary.sh `info` event type → top-of-summary INFO line
#   * reconcile::_drift_verdict_field — verdict-line field parser
#   * reconcile::_emit_drift_warning  — named WARNING row
#   * reconcile::_drift_disposition   — disposition fork (default + override)
#   * reconcile::_drift_prompt        — interactive prompt via the tty seam
#   * reconcile::_drift_worktree_lines — single-worktree collapse
# =============================================================================
...
# Portable hang-guard for the "never hangs" assertions.
#
...
# watcher that kills the command after N seconds — same time-bound, no
# external dependency.
#
...
_timeout() {
  ...
  # macOS). Mirrors coreutils `timeout`'s observable contract: never blocks
  # past `secs` (escalates TERM→KILL after a 1s grace), returns 124 on
  # timeout, and reaps both child + watcher.
  local marker
  marker="$(mktemp)"
  "$@" &
  local pid=$!
...
# =============================================================================
# Engine drift WARNING emitter + disposition fork (pure, network-free).
# =============================================================================
...
@test "_emit_drift_warning: names spec, disk, tracker, signals (phase-only fire)" {
  run bash -c "
    $_drift_src
    summary::start ''
  ...
  "
  [ "$status" -eq 0 ]
  # Renders in the existing warnings section (reuse `warned`).
  [[ "$output" == *"----- warnings -----"* ]]
...
# -----------------------------------------------------------------------------
# reconcile::_drift_disposition — default arm + non-interactive resolution
# -----------------------------------------------------------------------------

@test "_drift_disposition: non-interactive default (no flag, no TTY) resolves proceed" {
...
@test "non-interactive --on-drift unset → proceed (proceed-and-warn default)" {
...
@test "non-interactive --on-drift=proceed → proceed (writes + warns)" {
...
@test "non-interactive --on-drift=abort → abort (skip the drifted spec)" {
...
@test "non-interactive disposition NEVER hangs awaiting input" {
...
@test "--on-drift=abort wins over a TTY (override, no prompt) — empty-stdin proxy" {
...
@test "the --retroactive deprecation INFO row carries the verbatim text" {
  # The exact wording is contractual. Pin it so a future copy edit is a
...
@test "--retroactive sets no behavioral global (byte-identical disposition path)" {
  # With --retroactive the disposition global ARG_ON_DRIFT stays empty, so the
  # disposition resolution is byte-identical to omitting the flag.
  local with off
...
@test "a clean run that sets no warned events emits no warnings section" {
  run bash -c '
    source "'"$SUMMARY_SH"'"
    summary::start ""
    summary::add info "--retroactive is deprecated and now the default"
...
@test "--retroactive does not alter the disposition (convergence default)" {
...
@test "pre-existing-ahead spec, no flag, no TTY → default proceeds-and-warns" {
  # The proceed-and-warn default: a spec whose tracker issue is genuinely
  # ahead, reconciled non-interactively with no --on-drift, takes the
  # proceed-and-warn arm (writes disk state, keeps the WARNING row).
  run bash -c "
    $_drift_src
    summary::start ''
...
# =============================================================================
# Interactive prompt arm.
#
# reconcile::_drift_prompt drives the operator prompt via a redirectable tty
# seam (RECONCILE_DRIFT_TTY) so the prompt body is exercisable without a real
# terminal: p/proceed→proceed, a/abort/empty→abort, invalid re-prompts. The
# `[[ -t 0 ]]` gate itself can't be simulated in bats (stdin is always
# redirected), so the interactive-gate assertion is skipped with a reason; the
# prompt BODY is fully covered via the seam.
# =============================================================================
...
@test "interactive prompt — 'p' resolves proceed" {
  run _prompt $'p\n'
  [ "$status" -eq 0 ]
  [ "$output" = "proceed" ]
}

@test "interactive prompt — 'proceed' resolves proceed" {
  run _prompt $'proceed\n'
  [ "$status" -eq 0 ]
  [ "$output" = "proceed" ]
}

@test "interactive prompt — 'a' resolves abort" {
  run _prompt $'a\n'
  [ "$status" -eq 0 ]
  [ "$output" = "abort" ]
}

@test "interactive prompt — 'abort' resolves abort" {
  run _prompt $'abort\n'
  [ "$status" -eq 0 ]
  [ "$output" = "abort" ]
}

@test "interactive prompt — empty-enter resolves abort (safe default)" {
  run _prompt $'\n'
  [ "$status" -eq 0 ]
  [ "$output" = "abort" ]
}

@test "interactive prompt — invalid input re-prompts then resolves (no crash, no silent pick)" {
...
@test "interactive prompt — EOF (no answer) collapses to the safe abort (never hangs)" {
...
@test "prompt reads its tty seam, NOT the inherited stdin stream" {
...
@test "the [[ -t 0 ]] interactive GATE itself cannot be simulated in bats" {
  skip "bats always redirects stdin so [[ -t 0 ]] is false; the gate is exercised live (integration). The prompt BODY is fully covered above via the RECONCILE_DRIFT_TTY seam."
}

@test "--on-drift=abort skips a drifted spec (override) — no prompt on a TTY-less run" {
  run bash -c "$_drift_src; ARG_ON_DRIFT=abort; reconcile::_drift_disposition 005 'fired=1 phase_drift=1 recency_drift=0 signals=phase_ordering disk=planning linear=implementing' < /dev/null"
  [ "$status" -eq 0 ]
  [ "$output" = "abort" ]
}

@test "--on-drift=proceed writes + warns" {
  run bash -c "$_drift_src; ARG_ON_DRIFT=proceed; reconcile::_drift_disposition 005 'fired=1 phase_drift=1 recency_drift=0 signals=phase_ordering disk=planning linear=implementing' < /dev/null"
  [ "$status" -eq 0 ]
  [ "$output" = "proceed" ]
}

@test "an unrecognised --on-drift value is a usage error at parse time" {
  run _parse --all --on-drift=maybe
  [ "$status" -eq 2 ]
  [[ "$output" == *"--on-drift value must be abort or proceed"* ]]
}

@test "an abort disposition records ONLY the WARNING + skip rows (zero mutation proxy)" {
  ...
  # zero created/updated events for that spec.
  run bash -c "
    $_drift_src
    summary::start ''
    reconcile::_emit_drift_warning 005 'fired=1 phase_drift=1 recency_drift=0 signals=phase_ordering disk=planning linear=implementing'
    ...
    if [[ \"\$disp\" == abort ]]; then
      summary::add skipped 'spec 005 skipped by operator (backward-drift abort) — Jira unchanged'
    fi
...
@test "drift WARNING worktree lines collapse to nothing in the single-worktree case" {
  # _drift_worktree_lines returns empty when ≤1 worktree touches the spec.
  # Run outside a multi-worktree repo → no canonical lines.
