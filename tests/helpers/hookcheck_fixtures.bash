...
#
# PRIVACY (Principle IX): placeholder-only — the extension id `jira-sync`, the
# command `speckit.jira-sync.push`, and fabricated prompts/descriptions. NO real
...
_hookcheck_fx_jira_block() {
    ...
    printf '  %s:\n' "$hook"
    printf '  - extension: jira-sync\n'
    printf '    command: speckit.jira-sync.push\n'
    printf '    enabled: %s\n' "$enabled"
